#!/usr/bin/env bash
#
# Build a backfill inventory in-region via the stack's CodeBuild project.
#
# The DAAC's temporary S3 credentials only work from us-west-2, so running
# build_backfill_inventory.py locally with --access direct fails. This ships
# the committed repo (git archive HEAD) as the project's source zip, starts a
# build, and waits for it — every run is pinned by a commit plus the in-repo
# buildspec (scripts/inventory_buildspec.yml).
#
# Usage:
#   scripts/build_inventory_remote.sh -e ENV_FILE [-m MAX_COUNT] [-u S3_URI] [-n]
#
# Examples:
#   # 50-granule trial inventory for the hcho stack:
#   scripts/build_inventory_remote.sh -e .env_hcho -m 50
#   # Full inventory to an explicit key:
#   scripts/build_inventory_remote.sh -e .env_hcho \
#     -u s3://my-bucket/tempo/hcho/inventory/hcho.json
#   # See what would happen, without uploading or starting a build:
#   scripts/build_inventory_remote.sh -e .env_hcho -n
#
# COLLECTION and the default S3_URI are baked into the project by the CDK
# stack, so only the env file is required.

set -euo pipefail

usage() {
  cat >&2 <<'EOF'
Usage: build_inventory_remote.sh -e ENV_FILE [-m MAX_COUNT] [-u S3_URI] [-n]

  -e ENV_FILE   per-collection env file (e.g. .env_hcho); supplies the stack
                name, region, bucket, prefixes, and AWS profile
  -m MAX_COUNT  keep only the N most recent granules (trial runs)
  -u S3_URI     override the inventory destination baked into the project
  -n            dry run: resolve and print bucket/prefix/project/commit,
                then exit before uploading source or starting a build
EOF
  exit 2
}

ENV_FILE=""
MAX_COUNT=""
S3_URI=""
DRY_RUN=""
while getopts ":e:m:u:nh" opt; do
  case "$opt" in
    e) ENV_FILE="$OPTARG" ;;
    m) MAX_COUNT="$OPTARG" ;;
    u) S3_URI="$OPTARG" ;;
    n) DRY_RUN="1" ;;
    h) usage ;;
    :) echo "Error: -$OPTARG requires a value." >&2; usage ;;
    \?) echo "Error: unknown option -$OPTARG." >&2; usage ;;
  esac
done

[ -n "$ENV_FILE" ] && [ -f "$ENV_FILE" ] || { echo "Error: pass -e ENV_FILE." >&2; usage; }

env_get() {
  grep -E "^$1=" "$ENV_FILE" | tail -n1 | cut -d= -f2- | sed 's/^["'\'']//;s/["'\'']$//' || true
}

STACK="$(env_get STACK_NAME)"
REGION="$(env_get ACCOUNT_REGION)"
BUCKET="$(env_get ICECHUNK_BUCKET)"
PREFIX="$(env_get INVENTORY_PREFIX)"
[ -n "$PREFIX" ] || PREFIX="$(env_get S3_PREFIX)/inventory"
[ -n "$STACK" ] && [ -n "$BUCKET" ] || {
  echo "Error: STACK_NAME and ICECHUNK_BUCKET must be set in $ENV_FILE." >&2
  exit 2
}
if [ -z "${AWS_PROFILE:-}" ] && [ -n "$(env_get AWS_PROFILE)" ]; then
  export AWS_PROFILE="$(env_get AWS_PROFILE)"
fi
REGION_ARGS=()
[ -n "$REGION" ] && REGION_ARGS=(--region "$REGION")

# Which account do the current credentials actually resolve to, and does it
# match the env file's intent? Wrong-account runs are the classic SSO/profile
# mixup, so a mismatch aborts (real runs) or is reported (dry runs).
EXPECTED_ACCOUNT="$(env_get ACCOUNT_ID)"
CALLER_ACCOUNT="$(aws sts get-caller-identity --query Account --output text)" || {
  echo "Error: could not resolve credentials (aws sts get-caller-identity failed)." >&2
  echo "Check AWS_PROFILE (currently: ${AWS_PROFILE:-<none>}) and 'aws sso login'." >&2
  exit 1
}
CALLER_ARN="$(aws sts get-caller-identity --query Arn --output text)"
ACCOUNT_MISMATCH=""
if [ -n "$EXPECTED_ACCOUNT" ] && [ "$CALLER_ACCOUNT" != "$EXPECTED_ACCOUNT" ]; then
  ACCOUNT_MISMATCH="1"
fi
if [ -n "$ACCOUNT_MISMATCH" ] && [ -z "$DRY_RUN" ]; then
  echo "Error: credentials resolve to account $CALLER_ACCOUNT but $ENV_FILE sets ACCOUNT_ID=$EXPECTED_ACCOUNT." >&2
  echo "  Caller: $CALLER_ARN" >&2
  echo "Refusing to start a build in the wrong account." >&2
  exit 1
fi

PROJECT="$(aws cloudformation describe-stacks ${REGION_ARGS[@]+"${REGION_ARGS[@]}"} \
  --stack-name "$STACK" \
  --query "Stacks[0].Outputs[?OutputKey=='InventoryBuildProject'].OutputValue" \
  --output text)"
if [ -z "$PROJECT" ] || [ "$PROJECT" = "None" ]; then
  echo "Error: InventoryBuildProject output not found on stack '$STACK'." >&2
  exit 1
fi

DIRTY=""
[ -n "$(git status --porcelain)" ] && DIRTY="1"

if [ -n "$DRY_RUN" ]; then
  echo "== Dry run: no upload, no build started ==" >&2
  echo "Env file:          $ENV_FILE" >&2
  echo "Stack:              $STACK" >&2
  echo "Region:              ${REGION:-<default from AWS config>}" >&2
  echo "AWS profile:         ${AWS_PROFILE:-<none set>}" >&2
  if [ -n "$ACCOUNT_MISMATCH" ]; then
    echo "Account:             $CALLER_ACCOUNT  *** MISMATCH: $ENV_FILE expects $EXPECTED_ACCOUNT — a real run would abort ***" >&2
  else
    echo "Account:             $CALLER_ACCOUNT${EXPECTED_ACCOUNT:+ (matches ACCOUNT_ID in $ENV_FILE)}" >&2
  fi
  echo "Caller identity:     $CALLER_ARN" >&2
  echo "CodeBuild project:   $PROJECT" >&2
  echo "Code destination:    s3://$BUCKET/$PREFIX/source.zip" >&2
  if [ -n "$S3_URI" ]; then
    echo "Inventory dest.:     $S3_URI (from -u)" >&2
  else
    PROJECT_S3_URI="$(aws codebuild batch-get-projects ${REGION_ARGS[@]+"${REGION_ARGS[@]}"} \
      --names "$PROJECT" \
      --query "projects[0].environment.environmentVariables[?name=='S3_URI'].value | [0]" \
      --output text)"
    if [ -n "$PROJECT_S3_URI" ] && [ "$PROJECT_S3_URI" != "None" ]; then
      echo "Inventory dest.:     $PROJECT_S3_URI (project default; -u overrides)" >&2
    else
      echo "Inventory dest.:     <no S3_URI default found on the project — a run without -u may fail>" >&2
    fi
  fi
  echo "MAX_COUNT override:  ${MAX_COUNT:-<none>}" >&2
  echo "Commit to be built:  $(git rev-parse --short HEAD) ($(git log -1 --format=%s))" >&2
  if [ -n "$DIRTY" ]; then
    echo "Working tree:        DIRTY — uncommitted changes would NOT be included" >&2
  else
    echo "Working tree:        clean" >&2
  fi
  exit 0
fi

if [ -n "$DIRTY" ]; then
  echo "Warning: uncommitted changes are NOT included (the build runs git HEAD)." >&2
fi
ZIP="$(mktemp "${TMPDIR:-/tmp}/inventory-source.XXXXXX")"
trap 'rm -f "$ZIP"' EXIT
git archive --format=zip -o "$ZIP" HEAD
aws s3 cp "$ZIP" "s3://$BUCKET/$PREFIX/source.zip" ${REGION_ARGS[@]+"${REGION_ARGS[@]}"} >&2

OVERRIDES=(name=MAX_COUNT,value="$MAX_COUNT",type=PLAINTEXT)
[ -n "$S3_URI" ] && OVERRIDES+=(name=S3_URI,value="$S3_URI",type=PLAINTEXT)
BUILD_ID="$(aws codebuild start-build ${REGION_ARGS[@]+"${REGION_ARGS[@]}"} \
  --project-name "$PROJECT" \
  --environment-variables-override "${OVERRIDES[@]}" \
  --query 'build.id' --output text)"
echo "Started build $BUILD_ID (commit $(git rev-parse --short HEAD))" >&2

while :; do
  STATUS="$(aws codebuild batch-get-builds ${REGION_ARGS[@]+"${REGION_ARGS[@]}"} \
    --ids "$BUILD_ID" --query 'builds[0].buildStatus' --output text)"
  [ "$STATUS" = "IN_PROGRESS" ] || break
  sleep 30
done
echo "Build finished: $STATUS" >&2
if [ "$STATUS" != "SUCCEEDED" ]; then
  echo "Logs: aws logs tail /aws/codebuild/$PROJECT ${REGION_ARGS[*]:-}" >&2
  exit 1
fi
