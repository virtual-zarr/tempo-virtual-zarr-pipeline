...
**Recovery.** The store manifest and pending ledger are written through the same session as the data they describe, so they commit atomically with it and cannot drift from it or race a concurrent writer; no repair script is needed. A promote rejected by the compare-and-swap needs no repair either: nothing was consumed, and the next scheduled run retries against the new `main` tip.
