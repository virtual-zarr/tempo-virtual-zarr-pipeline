"""Pure helpers for the scheduled re-sort job.
...
inventory, find the first shifted axis position, rewrite the axis on a
``resort`` branch, and re-ingest every granule from that position on.
"""
...
def merge_pending(
    ...
    """Return the sorted union of the store manifest and the pending entries.

    The result is re-validated through the inventory model, so duplicate
    times or granule URs abort the job before it touches any branch.
    """
...
def first_shifted_index(manifest: BackfillInventory, merged: BackfillInventory) -> int:
    """Return the first axis position whose granule changed.

    Every slot from this position on must be rewritten: inserted granules
    occupy new positions, and existing granules after the earliest
    insertion shift by the number of insertions before them.
    """
